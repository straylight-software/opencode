...
import { createWeaponClient } from "@weapon-ai/sdk"
...
const { client, server } = createWeapon()
...
try {
  ...
  console.log("weapon session", session.id)
  if (shareId) {
...
function createWeapon() {
  ...
  const proc = spawn(`weapon`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createWeaponClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/weapon|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/weapon` or `/oc`")
  }
}
...
async function assertWeaponConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to weapon server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.weapon.ai" : "https://weapon.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.weapon.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("weapon-github-action")
    response = await fetch("https://api.weapon.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/weapon" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/weapon") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to weapon...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "weapon-agent[bot]"`
  await $`git config --global user.email "weapon-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `weapon/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/weapon-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[weapon session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
