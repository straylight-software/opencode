...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_enabled = option_env!("WEAPON_SQLITE").is_some();
    let sqlite_done = (sqlite_enabled && !sqlite_file_exists()).then(|| {
        tracing::info!(
            path = %weapon_db_path().expect("failed to get db path").display(),
                    ...
                    Some(
                        async move {
                            ...
                            if let Some(err) = err {
                                ...
                                return Err(format!(
                                    "Failed to spawn Weapon Server ({err}). Logs:\n{}",
                                    get_logs()
                                ));
                            }
...
fn get_sidecar_port() -> u32 {
    option_env!("WEAPON_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("WEAPON_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = weapon_db_path() else {
        return true;
    };
...
fn weapon_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("weapon").join("weapon.db"))
}
