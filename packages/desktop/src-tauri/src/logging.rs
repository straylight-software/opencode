...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("weapon-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("weapon_lib=debug,weapon_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("weapon_lib=info,weapon_desktop=info,sidecar=info")
        }
    });
