...
With Weapon you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Weapon uses our own `weapon` theme.
...
Weapon comes with several built-in themes.
...
Weapon supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/weapon/themes/*.json` or `$XDG_CONFIG_HOME/weapon/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.weapon/themes/*.json`
4. **Current working directory** - Defined in `./.weapon/themes/*.json`
