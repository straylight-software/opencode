...
工具允許 LLM 在您的程式碼庫中執行操作。 weapon 附帶了一組內建工具，但您可以使用 [自定義工具](/docs/custom-tools) 或 [MCP 伺服器](/docs/mcp-servers) 對其進行擴展。
...
:::note
該工具僅在 `WEAPON_EXPERIMENTAL_LSP_TOOL=true`（或 `WEAPON_EXPERIMENTAL=true`）時可用。
:::
...
:::note
僅當使用 weapon 供應商或 `WEAPON_ENABLE_EXA` 環境變數設定為任何真值（例如 `true` 或 `1`）時，此工具才可用。
