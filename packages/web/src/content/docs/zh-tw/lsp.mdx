---
title: LSP 伺服器
description: weapon 與您的 LSP 伺服器整合。
---

weapon 與您的語言伺服器協定 (LSP) 整合，以幫助 LLM 與您的程式碼庫互動。它使用診斷向 LLM 提供回饋。

---

## 內建

weapon 附帶了多種適用於流行語言的內建 LSP 伺服器：
...
:::note
您可以透過將 `WEAPON_DISABLE_LSP_DOWNLOAD` 環境變數設定為 `true` 來禁用自動 LSP 伺服器下載。
:::
...
您可以透過 weapon 設定中的 `lsp` 部分自定義 LSP 伺服器。
