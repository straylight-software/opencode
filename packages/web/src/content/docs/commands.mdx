...
Create `.weapon/commands/test.md`:
...
You can add custom commands through the Weapon config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Weapon [config](/docs/config):
...
weapon includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
