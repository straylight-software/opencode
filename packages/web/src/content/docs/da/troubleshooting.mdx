...
For at fejlfinde problemer med Weapon, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\weapon\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `weapon --log-level DEBUG`.

---

## Lagring

weapon gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/weapon/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\weapon`
...
Weapon Desktop kører en lokal Weapon-server (`weapon-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Weapon`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/weapon/weapon.jsonc` (eller `~/.config/weapon/weapon.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/weapon/weapon.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\weapon\weapon.jsonc`
...
Weapon kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/weapon/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\weapon\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Weapon kan genopbygge den.

1. Afslut Weapon Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/weapon`
- **Linux**: slet `~/.cache/weapon` (eller kør `rm -rf ~/.cache/weapon`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\weapon`

3. Start Weapon Desktop på ny.

---

### Løs problemer med serverforbindelse

Weapon Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `weapon.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `WEAPON_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `WEAPON_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Weapon Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Weapon.

---

### Meddelelser vises ikke

Weapon Desktop viser kun systemvarsler når:

- varsler er aktiveret for Weapon i dine OS-indstillinger, og
...
1. Afslut Weapon Desktop.
2. Find og slet disse filer (de findes i Weapon Desktop-appens datamappe):

- `weapon.settings.dat` (skrivebordsstandardserver URL)
- `weapon.global.dat` og `weapon.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Weapon:
...
For at finde ud af hvilke modeller du har adgang til, kør `weapon models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\weapon`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. weapon installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\weapon`

2. Start weapon på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge weapon til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
weapon vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
