---
title: Server
description: Interager med weapon-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `weapon serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en weapon-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `weapon.local` |
...
Indstil `WEAPON_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `weapon`, eller indstil `WEAPON_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `weapon serve` og `weapon web`.
...
Når du kører `weapon` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug weapon-serveren til at interagere med weapon programmatisk.
:::

Denne arkitektur lader weapon understøtte flere klienter og lader dig interagere med weapon programmatisk.

Du kan køre `weapon serve` for at starte en selvstændig server. Hvis du har
weapon TUI kørende, vil `weapon serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Weapon [IDE](/docs/ide) plugins.
...
Weapon-serveren viser følgende API'er.
