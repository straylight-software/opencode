...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i weapon-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i weapon giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem weapon-konfigurationen.
...
weapon leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.weapon/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `weapon.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger weapon modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale weapon-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.weapon/modes/` for projektspecifikke tilstande eller `~/.config/weapon/modes/` for globale tilstande:
