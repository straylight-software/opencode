---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Weapon.
---

import config from "../../../../config.mjs"
export const console = config.console

Weapon bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Weapon-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/weapon/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Weapon-konfigurationen.
...
Weapon Zen er en liste over modeller leveret af Weapon-teamet, som er blevet testet og verificeret til at fungere godt med Weapon. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Weapon Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg weapon og gå til [weapon.ai/auth](https://weapon.ai/auth).
...
Det fungerer som alle andre udbydere i Weapon og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med Weapon:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører weapon:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `weapon.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Weapon understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at weapon skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem weapon-konfigurationen.
...
gemme tokenet i weapon auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Weapon bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Weapon
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`weapon.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Weapon:
...
Sådan bruger du Google Vertex AI med Weapon:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem weapon, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [weapon-helicone-session](https://github.com/H2Shami/weapon-helicone-session) plugin for automatisk at logge hver Weapon-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Weapon-samtale opført som en separat session.
...
Du kan konfigurere weapon til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere weapon til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere weapon til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Weapon. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/weapon) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Weapon:
...
3. Kopier API-nøglen til brug i Weapon.
...
6. **Vigtigt**: Før du bruger skymodeller i Weapon, skal du hente modelinformationen lokalt:
...
Weapon Zen er en liste over testede og verificerede modeller leveret af Weapon-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Weapon Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Weapon Zen**.

   ```txt
   /connect
   ```

3. Indtast Weapon API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem weapon-konfigurationen.
...
5. Du kan også tilpasse dem gennem weapon-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Weapon:
...
Du kan også tilpasse modeller gennem weapon-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med weapon. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `weapon.json`-filen i projektkataloget:
...
`limit`-felterne lader Weapon forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `weapon auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek weapon-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i weapon-konfigurationen.
