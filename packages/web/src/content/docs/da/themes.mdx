...
Med Weapon kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Weapon vårt eget `weapon`-tema.
...
Weapon støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/weapon/themes/*.json` eller `$XDG_CONFIG_HOME/weapon/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.weapon/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.weapon/themes/*.json`
