---
title: Plugins
description: Skriv dine egne plugins for at udvide Weapon.
---

Plugins giver dig mulighed for at udvide Weapon ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Weapons standardadfærd.
...
- `.weapon/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/weapon/node_modules/`.
...
1. Global konfiguration (`~/.config/weapon/weapon.json`)
2. Projektkonfiguration (`weapon.json`)
3. Global plugin-mappe (`~/.config/weapon/plugins/`)
4. Projekt plugin bibliotek (`.weapon/plugins/`)
...
Weapon kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En weapon SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide weapon.
...
:::note
Hvis du bruger Weapon desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå weapon i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til weapon:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som weapon kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for weapon sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://weapon.ai/docs/sdk) for detaljer.
