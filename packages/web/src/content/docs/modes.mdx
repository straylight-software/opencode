...
:::caution
Modes are now configured through the `agent` option in the weapon config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in weapon allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the weapon config.
...
weapon comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.weapon/plans/*.md` to detail the plan itself
...
Configure modes in your `weapon.json` config file:
...
both the global weapon config and the project specific config.
...
Create mode files in `.weapon/modes/` for project-specific modes or `~/.config/weapon/modes/` for global modes:
