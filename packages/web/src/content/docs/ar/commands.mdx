...
أنشئ الملف `.weapon/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Weapon أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Weapon:
...
- على مستوى النظام: `~/.config/weapon/commands/`
- لكل مشروع: `.weapon/commands/`
...
يتضمن weapon عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
