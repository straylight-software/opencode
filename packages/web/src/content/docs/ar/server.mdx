---
title: الخادم
description: تفاعل مع خادم weapon عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `weapon serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل weapon استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `weapon.local` |
...
عيّن `WEAPON_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `weapon`، أو عيّن `WEAPON_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `weapon serve` و `weapon web`.
...
عند تشغيل `weapon` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم weapon للتفاعل مع weapon برمجيا.
:::

تتيح هذه البنية لـ weapon دعم عدة عملاء وتمكّنك من التفاعل مع weapon برمجيا.

يمكنك تشغيل `weapon serve` لبدء خادم مستقل. إذا كانت واجهة weapon في terminal (TUI)
قيد التشغيل، فسيبدأ `weapon serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Weapon لـ [IDE](/docs/ide).
...
يعرض خادم weapon واجهات API التالية.
