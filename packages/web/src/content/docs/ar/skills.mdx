...
تتيح مهارات الوكيل لـ Weapon اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Weapon في هذه المواقع:

- إعدادات المشروع: `.weapon/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/weapon/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Weapon صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.weapon/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/weapon/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.weapon/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Weapon المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `weapon.json`:
...
**للوكلاء المدمجين** (في `weapon.json`):
