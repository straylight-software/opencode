...
مع Weapon يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Weapon سمتنا `weapon`.
...
- يريدون أن يطابق Weapon مظهر terminal لديهم
...
يدعم Weapon نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/weapon/themes/*.json` أو `$XDG_CONFIG_HOME/weapon/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.weapon/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.weapon/themes/*.json`
