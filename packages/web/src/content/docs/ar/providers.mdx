---
title: المزوّدون
description: استخدام أي موفّر LLM في Weapon.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Weapon ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Weapon.
...
في `~/.local/share/weapon/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Weapon.
...
Weapon Zen هي قائمة نماذج يوفّرها فريق Weapon وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Weapon. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Weapon Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر weapon، ثم انتقل إلى [weapon.ai/auth](https://weapon.ai/auth).
...
يعمل مثل أي مزوّد آخر في Weapon واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع Weapon:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل weapon:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `weapon.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Weapon غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل weapon بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات weapon.
...
تخزين الرمز في مساحة تخزين مصادقة weapon.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Weapon نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Weapon
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`weapon.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع weapon:
...
لاستخدام Google Vertex AI مع Weapon:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر weapon، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [weapon-helicone-session](https://github.com/H2Shami/weapon-helicone-session) لتسجيل كل محادثة Weapon تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Weapon مدرجة كجلسة مستقلة.
...
يمكنك ضبط weapon لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط weapon لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط weapon لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Weapon. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/weapon) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Weapon:
...
3. انسخ مفتاح API لاستخدامه في Weapon.
...
6. **مهم**: قبل استخدام نماذج السحابة في Weapon، يجب سحب معلومات النموذج محليا:
...
Weapon Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Weapon. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Weapon Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Weapon Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Weapon.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات weapon.
...
5. يمكنك أيضا تخصيصها عبر إعدادات weapon. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Weapon:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات weapon. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع weapon. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `weapon.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Weapon بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `weapon auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات weapon وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات weapon.
