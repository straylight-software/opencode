...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di weapon. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in weapon ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di weapon.
...
weapon include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.weapon/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `weapon.json`:
...
- Globale: `~/.config/weapon/modes/`
- Progetto: `.weapon/modes/`
...
Se non viene specificata alcuna temperatura, weapon usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di weapon sia per quella specifica del progetto.
...
Crea file di modalita in `.weapon/modes/` per modalita specifiche del progetto oppure in `~/.config/weapon/modes/` per modalita globali:
