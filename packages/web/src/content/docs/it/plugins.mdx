---
title: Plugin
description: Scrivi plugin per estendere Weapon.
---

I plugin ti permettono di estendere Weapon agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di Weapon.
...
- `.weapon/plugins/` - plugin a livello progetto
- `~/.config/weapon/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/weapon/node_modules/`.
...
1. Config globale (`~/.config/weapon/weapon.json`)
2. Config di progetto (`weapon.json`)
3. Directory plugin globale (`~/.config/weapon/plugins/`)
4. Directory plugin di progetto (`.weapon/plugins/`)
...
Weapon esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di weapon per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere weapon.
...
:::note
Se usi l'app desktop di Weapon, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a weapon di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a weapon:
...
L'helper `tool` crea uno strumento personalizzato che weapon puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in weapon insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://weapon.ai/docs/sdk) per i dettagli.
