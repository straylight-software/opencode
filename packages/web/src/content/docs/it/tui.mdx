---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Weapon.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Weapon fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Weapon avvia la TUI per la directory corrente.
...
Quando usi la TUI di Weapon, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Weapon. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Puoi personalizzare il comportamento della TUI tramite il file di config di Weapon.
