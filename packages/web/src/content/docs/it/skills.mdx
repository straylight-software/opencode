...
Le skill degli agenti permettono a Weapon di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Weapon cerca in queste posizioni:

- Config di progetto: `.weapon/skills/<name>/SKILL.md`
- Config globale: `~/.config/weapon/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Weapon risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.weapon/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/weapon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.weapon/skills/git-release/SKILL.md` cosi':
...
Weapon elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `weapon.json`:
...
**Per agenti integrati** (in `weapon.json`):
