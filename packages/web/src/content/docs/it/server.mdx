---
title: Server
description: Interagisci con il server weapon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `weapon serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client weapon.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `weapon.local` |
...
Imposta `WEAPON_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `weapon`, oppure imposta `WEAPON_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `weapon serve` sia per `weapon web`.
...
Quando esegui `weapon` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server weapon per interagire con weapon in modo programmatico.
:::

Questa architettura permette a weapon di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `weapon serve` per avviare un server standalone. Se la TUI di weapon e gia in esecuzione, `weapon serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Weapon.
...
Il server weapon espone le seguenti API.
