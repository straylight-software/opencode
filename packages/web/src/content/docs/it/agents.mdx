...
In Weapon esistono due tipi di agenti: agenti primari e subagenti.
...
Weapon include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Weapon include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Weapon include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `weapon.json`:
...
- Globale: `~/.config/weapon/agents/`
- Per progetto: `.weapon/agents/`
...
Se non specifichi una temperature, Weapon usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Weapon sia per la config specifica del progetto.
...
L'ID modello nella configurazione Weapon usa il formato `provider/model-id`. Per esempio, se stai usando [Weapon Zen](/docs/zen), useresti `weapon/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `weapon models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/weapon).
:::
