...
ove ili konfigurirajte svoje putem weapon config.
...
weapon dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.weapon/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `weapon.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/weapon/modes/`
- Projekat: `.weapon/modes/`
...
Ako temperatura nije navedena, weapon koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.weapon/modes/` za specifične načine rada ili `~/.config/weapon/modes/` za globalne načine:
