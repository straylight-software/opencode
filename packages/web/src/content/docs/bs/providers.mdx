---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Weapon.
---

import config from "../../../../config.mjs"
export const console = config.console

Weapon koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Weapon konfiguraciji.
...
u `~/.local/share/weapon/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Weapon
config.
...
Weapon Zen je lista modela koje je obezbedio Weapon tim koji su bili
testirano i potvrđeno da dobro radi sa Weapon. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Weapon Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite weapon i idite na [weapon.ai/auth](https://weapon.ai/auth).
...
Radi kao i svaki drugi provajder u Weapon i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s Weapon:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete weapon:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `weapon.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Weapon.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi weapon ispravno radio.
   :::
...
Također možete dodati modele kroz svoju weapon konfiguraciju.
...
da pohrani token u weapon auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Weapon koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Weapon
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`weapon.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s weapon:
...
Za korištenje Google Vertex AI s Weapon:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće weapon.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem weapona, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [weapon-helicone-session](https://github.com/H2Shami/weapon-helicone-session) da automatski prijavite svaki Weapon razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Weapon razgovor naveden kao zasebna sesija.
...
Možete konfigurirati weapon za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati weapon za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati weapon za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Weapon. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/weapon) za detalje.
:::
...
Da biste koristili Ollama Cloud s Weapon:
...
3. Kopirajte API ključ za korištenje u Weapon.
...
6. **Važno**: Prije upotrebe modela oblaka u Weapon, morate lokalno povući informacije o modelu:
...
Weapon Zen je lista testiranih i verifikovanih modela koju je obezbedio Weapon tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Weapon Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Weapon Zen**.

```txt
   /connect
```

3. Unesite svoj Weapon API ključ.
...
Također možete dodati dodatne modele putem vaše weapon konfiguracije.
...
5. Također ih možete prilagoditi putem vaše weapon konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Weaponom:
...
Također možete prilagoditi modele kroz svoju weapon konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s weapon-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `weapon.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Weapon da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `weapon auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Weapon konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj weapon konfiguraciji.
