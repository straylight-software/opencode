...
Da biste otklonili probleme s Weapon, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\weapon\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `weapon --log-level DEBUG`.

---

## Pohrana

Weapon pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/weapon/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\weapon`
...
Weapon Desktop pokreće lokalni Weapon server (`weapon-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Weapon` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/weapon/weapon.jsonc` (ili `~/.config/weapon/weapon.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/weapon/weapon.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\weapon\weapon.jsonc`
...
Weapon također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/weapon/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\weapon\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Weapon mogao ponovo izgraditi.

1. Potpuno zatvorite Weapon Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/weapon`
- **Linux**: obrišite `~/.cache/weapon` (ili pokrenite `rm -rf ~/.cache/weapon`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\weapon`

3. Ponovo pokrenite Weapon Desktop.

---

### Rješavanje problema sa vezom na serveru

Weapon Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `weapon.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `WEAPON_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `WEAPON_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Weapon Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Weapon karakteristikama.

---

### Obavještenja se ne prikazuju

Weapon Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Weapon u postavkama vašeg OS-a, i
...
1. Zatvorite Weapon Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Weapon Desktop aplikacije):

- `weapon.settings.dat` (URL zadanog servera za desktop)
- `weapon.global.dat` i `weapon.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `weapon models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\weapon`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Weapon dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\weapon`

2. Ponovo pokrenite Weapon da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Weapon da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Weapon će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
