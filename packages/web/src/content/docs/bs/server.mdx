---
title: Server
description: Komunicirajte s weapon serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `weapon serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji weapon klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `weapon.local` |
...
Postavite `WEAPON_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `weapon`, ili postavite `WEAPON_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `weapon serve` i za `weapon web`.
...
Kada pokrenete `weapon`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite weapon server za programsku interakciju sa weapon-om.
:::

Ova arhitektura omogucava weapon podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `weapon serve` da startate standalone server. Ako je weapon TUI vec pokrenut, `weapon serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Weapon [IDE](/docs/ide) pluginovi.
...
weapon server izlaže sljedece API-je.
