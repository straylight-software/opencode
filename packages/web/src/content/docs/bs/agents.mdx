...
Postoje dvije vrste agenata u Weapon: primarni agenti i podagenti.
...
Weapon dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Weapon dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Weapon dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `weapon.json`:
...
- Globalno: `~/.config/weapon/agents/`
- Po projektu: `.weapon/agents/`
...
Ako temperatura nije navedena, Weapon koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Weapon konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Weapon konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Weapon Zen](/docs/zen), koristili biste `weapon/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `weapon models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/weapon).
:::
