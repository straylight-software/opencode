...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Weapon dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Weapon.
...
:::note
Ovaj alat je dostupan samo kada je `WEAPON_EXPERIMENTAL_LSP_TOOL=true` (ili `WEAPON_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Weapon provajdera ili kada je varijabla `WEAPON_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Weapon:
