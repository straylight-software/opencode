...
Agent skills omogucavaju Weapon da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Weapon pretrazuje ove lokacije:

- Konfiguracija projekta: `.weapon/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/weapon/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Weapon ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.weapon/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/weapon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.weapon/skills/git-release/SKILL.md` ovako:
...
Weapon navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `weapon.json`:
...
**Za ugradene agente** (u `weapon.json`):
