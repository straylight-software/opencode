...
U Weapon mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Weapon koristi nasu `weapon` temu.
...
Weapon dolazi sa vise ugradenih tema.
...
- Zele da Weapon odgovara izgledu njihovog terminala
...
Weapon podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/weapon/themes/*.json` ili `$XDG_CONFIG_HOME/weapon/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.weapon/themes/*.json`
4. **Trenutni radni direktorij** - `./.weapon/themes/*.json`
