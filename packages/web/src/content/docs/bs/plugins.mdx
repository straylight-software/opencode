---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Weapon.
---

Dodaci vam omogućavaju da proširite Weapon spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Weapon.
...
- `.weapon/plugins/` - Dodaci na nivou projekta
- `~/.config/weapon/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/weapon/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/weapon/weapon.json`)
2. Konfiguracija projekta (`weapon.json`)
3. Globalni direktorij dodataka (`~/.config/weapon/plugins/`)
4. Direktorij dodataka projekta (`.weapon/plugins/`)
...
Weapon pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Weapon SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Weapon.
...
:::note
Ako koristite desktop aplikaciju Weapon, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite weapon da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u weapon:
...
Pomoćnik `tool` kreira prilagođeni alat koji weapon može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za weapon zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://weapon.ai/docs/sdk) za detalje.
