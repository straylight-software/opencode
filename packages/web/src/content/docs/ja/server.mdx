---
title: サーバー
description: HTTP 経由で Weapon サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`weapon serve` コマンドは、Weapon クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `weapon.local` |
...
HTTP 基本認証でサーバーを保護するには、`WEAPON_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `weapon` になるか、`WEAPON_SERVER_USERNAME` を設定してオーバーライドします。これは、`weapon serve` と `weapon web` の両方に当てはまります。
...
`weapon` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Weapon サーバーを使用して、プログラムで Weapon と対話します。
:::

`weapon serve` を実行してスタンドアロンサーバーを起動できます。すでに Weapon TUI を実行している場合でも、`weapon serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Weapon [IDE](/docs/ide) プラグインによって使用されます。
