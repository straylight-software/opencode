---
title: Servidor
description: Interaja com o servidor weapon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `weapon serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente weapon pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `weapon.local` |
...
Defina `WEAPON_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `weapon`, ou defina `WEAPON_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `weapon serve` quanto ao `weapon web`.

```bash
WEAPON_SERVER_PASSWORD=your-password weapon serve
```
...
Quando você executa `weapon`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor weapon para interagir com o weapon programaticamente.
:::

Essa arquitetura permite que o weapon suporte múltiplos clientes e permite que você interaja com o weapon programaticamente.

Você pode executar `weapon serve` para iniciar um servidor autônomo. Se você tiver o TUI do weapon em execução, `weapon serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do weapon [IDE](/docs/ide).
...
O servidor weapon expõe as seguintes APIs.
