---
title: Plugins
description: Escreva seus próprios plugins para estender o weapon.
---

Plugins permitem que você estenda o weapon conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do weapon.
...
- `.weapon/plugins/` - Plugins em nível de projeto
- `~/.config/weapon/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/weapon/node_modules/`.
...
1. Configuração global (`~/.config/weapon/weapon.json`)
2. Configuração do projeto (`weapon.json`)
3. Diretório de plugins global (`~/.config/weapon/plugins/`)
4. Diretório de plugins do projeto (`.weapon/plugins/`)
...
O weapon executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do weapon para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o weapon.
...
:::note
Se você estiver usando o aplicativo desktop weapon, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o weapon de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao weapon:
...
O helper `tool` cria uma ferramenta personalizada que o weapon pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o weapon junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://weapon.ai/docs/sdk) para detalhes.
