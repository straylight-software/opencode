...
Com o weapon, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o weapon usa nosso próprio tema `weapon`.
...
O weapon vem com vários temas embutidos.
...
- Querem que o weapon corresponda à aparência do seu terminal
...
O weapon suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/weapon/themes/*.json` ou `$XDG_CONFIG_HOME/weapon/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.weapon/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.weapon/themes/*.json`
