...
As habilidades do agente permitem que o weapon descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O weapon pesquisa nesses locais:

- Configuração do projeto: `.weapon/skills/<name>/SKILL.md`
- Configuração global: `~/.config/weapon/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o weapon sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.weapon/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/weapon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.weapon/skills/git-release/SKILL.md` assim:
...
O weapon lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `weapon.json`:
...
**Para agentes embutidos** (em `weapon.json`):
