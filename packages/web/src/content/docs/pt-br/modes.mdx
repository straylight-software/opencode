...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do weapon. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no weapon permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do weapon.
...
O weapon vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.weapon/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `weapon.json`:
...
- Global: `~/.config/weapon/modes/`
- Projeto: `.weapon/modes/`

```markdown title="~/.config/weapon/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o weapon usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do weapon quanto para a configuração específica do projeto.
...
Crie arquivos de modo em `.weapon/modes/` para modos específicos do projeto ou `~/.config/weapon/modes/` para modos globais:

```markdown title=".weapon/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/weapon/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
