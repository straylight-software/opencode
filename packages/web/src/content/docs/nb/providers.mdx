---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Weapon.
---

import config from "../../../../config.mjs"
export const console = config.console

Weapon bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Weapon-konfigurasjonen.
...
i `~/.local/share/weapon/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Weapon
konfig.
...
Weapon Zen er en liste over modeller levert av Weapon-teamet som har vært
testet og verifisert for å fungere godt med Weapon. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Weapon Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg weapon og gå til [weapon.ai/auth](https://weapon.ai/auth).
...
Det fungerer som alle andre leverandører i Weapon og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med Weapon:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Weapon:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `weapon.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i weapon støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Weapon skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Weapon-konfigurasjonen.
...
for å lagre token i Weapon auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Weapon bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Weapon
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`weapon.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med Weapon:
...
Slik bruker du Google Vertex AI med Weapon:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Weapon.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Weapon, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [weapon-helicone-session](https://github.com/H2Shami/weapon-helicone-session) plugin for automatisk å logge hver Weapon-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Weapon-samtale oppført som en separat økt.
...
Du kan konfigurere Weapon for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Weapon for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Weapon for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Weapon. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/weapon) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Weapon:
...
3. Kopier API-nøkkelen for bruk i Weapon.
...
6. **Viktig**: Før du bruker skymodeller i Weapon, må du hente modellinformasjonen lokalt:
...
Weapon Zen er en liste over testede og verifiserte modeller levert av Weapon-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Weapon Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Weapon Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Weapon API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Weapon-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Weapon-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Weapon:
...
Du kan også tilpasse modeller gjennom Weapon-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Weapon. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `weapon.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Weapon forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `weapon auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Weapon-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Weapon-konfigurasjonen.
