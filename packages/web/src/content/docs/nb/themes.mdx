...
Med weapon kan du velge fra ett av flere innebygde temaer, bruke et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruker weapon vårt eget `weapon`-tema.
...
weapon kommer med flere innebygde temaer.
...
- Vil at weapon skal matche terminalens utseende
...
weapon støtter et fleksibelt JSON-basert temasystem som lar brukere enkelt lage og tilpasse temaer.
...
2. **Brukerkonfigurasjonskatalog** - Definert i `~/.config/weapon/themes/*.json` eller `$XDG_CONFIG_HOME/weapon/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.weapon/themes/*.json`
4. **Gjeldende arbeidskatalog** - Definert i `./.weapon/themes/*.json`
