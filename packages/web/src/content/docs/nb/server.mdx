---
title: Tjener
description: Samhandle med Weapon-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `weapon serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Weapon-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `weapon.local` |
...
Sett `WEAPON_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `weapon` som standard, eller sett `WEAPON_SERVER_USERNAME` for å overstyre det. Dette gjelder både `weapon serve` og `weapon web`.
...
Når du kjører `weapon` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Weapon-serveren til å samhandle med Weapon programmatisk.
:::

Denne arkitekturen lar Weapon støtte flere klienter og lar deg samhandle med Weapon programmatisk.

Du kan kjøre `weapon serve` for å starte en frittstående server. Hvis du har
Weapon TUI kjører, vil `weapon serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Weapon [IDE](/docs/ide) plugins.
...
weapon-serveren viser følgende APIer.
