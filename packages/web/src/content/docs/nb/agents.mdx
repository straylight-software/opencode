...
Det er to typer agenter i Weapon; primære agenter og underagenter.
...
Weapon kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Weapon kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Weapon kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `weapon.json`:
...
- Globalt: `~/.config/weapon/agents/`
- Per prosjekt: `.weapon/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Weapon modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Weapon-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Weapon-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Weapon Zen](/docs/zen), vil du bruke `weapon/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `weapon models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/weapon).
:::
