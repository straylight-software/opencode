...
description: Skriv dine egne programtillegg for å utvide weapon.
---

Plugins lar deg utvide Weapon ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Weapon.
...
- `.weapon/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/weapon/node_modules/`.
...
1. Global konfigurasjon (`~/.config/weapon/weapon.json`)
2. Prosjektkonfigurasjon (`weapon.json`)
3. Global plugin-katalog (`~/.config/weapon/plugins/`)
4. Prosjektpluginkatalog (`.weapon/plugins/`)
...
Weapon kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Weapon SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Weapon.
...
:::note
Hvis du bruker Weapon-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Weapon fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Weapon:
...
`tool`-hjelperen lager et tilpasset verktøy som Weapon kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Weapon sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://weapon.ai/docs/sdk) for detaljer.
