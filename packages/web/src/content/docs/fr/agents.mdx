...
Il existe deux types d'agents dans Weapon ; agents primaires et sous-agents.
...
Weapon est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Weapon est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Weapon est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `weapon.json` :
...
- Global : `~/.config/weapon/agents/`
- Par projet : `.weapon/agents/`

```markdown title="~/.config/weapon/agents/review.md"
---
...
Si aucune température n'est spécifiée, Weapon utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Weapon et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Weapon utilise le format `provider/model-id`. Par exemple, si vous utilisez [Weapon Zen](/docs/zen), vous utiliserez `weapon/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="weapon.json" {3-6,9-12}
{
  "$schema": "https://weapon.ai/config.json",
  "tools": {
    "write": true,
...
```json title="weapon.json" {3-5,8-10}
{
  "$schema": "https://weapon.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `weapon models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/weapon).
:::
...
```markdown title="~/.config/weapon/agents/docs-writer.md"
---
...
```markdown title="~/.config/weapon/agents/security-auditor.md"
---
