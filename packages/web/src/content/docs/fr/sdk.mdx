---
title: SDK
description: Client JS de type sécurisé pour le serveur weapon.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK weapon JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler weapon par programme.
...
Créez une instance de weapon :

```javascript
import { createWeapon } from "@weapon-ai/sdk"

const { client } = await createWeapon()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `weapon.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${weapon.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de weapon, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@weapon-ai/sdk"
```
