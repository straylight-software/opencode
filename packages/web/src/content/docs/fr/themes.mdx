...
Avec Weapon, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Weapon utilise notre propre thème `weapon`.
...
Weapon est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Weapon corresponde à l'apparence de leur terminal
...
Weapon prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/weapon/themes/*.json` ou `$XDG_CONFIG_HOME/weapon/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.weapon/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.weapon/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/weapon/themes
vim ~/.config/weapon/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .weapon/themes
vim .weapon/themes/my-theme.json
```
...
{
  "$schema": "https://weapon.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
