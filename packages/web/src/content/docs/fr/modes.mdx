...
:::caution
Les modes sont désormais configurés via l'option `agent` dans la configuration weapon.
L’option `mode` est désormais obsolète. [En savoir plus](/docs/agents).
:::

Les modes dans weapon vous permettent de personnaliser le comportement, les outils et les prompts pour différents cas d'utilisation.

Il est livré avec deux modes intégrés : **build** et **plan**. Vous pouvez personnaliser ceux-ci ou configurez les vôtres via la configuration weapon.
...
weapon est livré avec deux modes intégrés.
...
- `edit` - Impossible de modifier les fichiers existants, à l'exception des fichiers situés à `.weapon/plans/*.md` pour détailler le plan lui-même
...
Configurez les modes dans votre fichier de configuration `weapon.json` :
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Donc ça marche pour à la fois la configuration globale weapon et la configuration spécifique au projet.
...
Créez des fichiers de mode dans `.weapon/modes/` pour les modes spécifiques au projet ou `~/.config/weapon/modes/` pour les modes globaux :
