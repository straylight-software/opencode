---
title: Serveur
description: Interagissez avec le serveur weapon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `weapon serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client weapon peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `weapon.local` |
...
Définissez `WEAPON_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `weapon`, ou définissez `WEAPON_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `weapon serve` et à `weapon web`.
...
Lorsque vous exécutez `weapon`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur weapon pour interagir avec weapon par programmation.
:::

Cette architecture permet à weapon de prendre en charge plusieurs clients et vous permet d'interagir avec weapon par programme.

Vous pouvez exécuter `weapon serve` pour démarrer un serveur autonome. Si vous avez le
weapon TUI en cours d'exécution, `weapon serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Weapon [IDE](/docs/ide).
...
Le serveur weapon expose les API suivantes.
