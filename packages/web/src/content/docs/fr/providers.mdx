---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Weapon.
---

import config from "../../../../config.mjs"
export const console = config.console

Weapon utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Weapon.
...
en `~/.local/share/weapon/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Weapon
configuration.
...
Weapon Zen est une liste de modèles fournis par l'équipe Weapon qui ont été
testé et vérifié pour fonctionner correctement avec Weapon. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Weapon Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez weapon et dirigez-vous vers [weapon.ai/auth](https://weapon.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Weapon et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec Weapon :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de weapon :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY weapon

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile weapon

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX weapon
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `weapon.json` :

```json title="weapon.json"
{
  "$schema": "https://weapon.ai/config.json",
  "provider": {
    "amazon-bedrock": {
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Weapon n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que weapon fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX weapon
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX weapon
   ```
...
Vous pouvez également ajouter des modèles via votre configuration weapon.
...
pour stocker le jeton dans le stockage d'authentification weapon.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Weapon utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Weapon
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `weapon.json`. Il est également recommandé de désactiver le partage de session.
...
{
  "$schema": "https://weapon.ai/config.json",
  "plugin": ["@gitlab/weapon-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec weapon :
...
Pour utiliser Google Vertex AI avec Weapon :
...
Définissez-les lors de l'exécution de weapon.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id weapon
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via weapon, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/weapon/weapon.jsonc"
{
  "$schema": "https://weapon.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [weapon-helicone-session](https://github.com/H2Shami/weapon-helicone-session) pour enregistrer automatiquement chaque conversation Weapon en tant que session dans Helicone.

```bash
npm install -g weapon-helicone-session
```
...
```json title="weapon.json"
{
  "plugin": ["weapon-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Weapon répertoriée comme une session distincte.
...
Vous pouvez configurer weapon pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="weapon.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://weapon.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer weapon pour utiliser des modèles locaux via LM Studio.

```json title="weapon.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://weapon.ai/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer weapon pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Weapon. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/weapon) pour plus de détails.
:::

```json title="weapon.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://weapon.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec Weapon :
...
3. Copiez la clé API à utiliser dans Weapon.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Weapon, vous devez extraire les informations du modèle localement :
...
Weapon Zen est une liste de modèles testés et vérifiés fournie par l'équipe Weapon. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Weapon Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Weapon Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Weapon API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration weapon.
...
5. Vous pouvez également les personnaliser via votre configuration weapon. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' weapon
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group weapon
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Weapon :
...
Vous pouvez également personnaliser les modèles via votre configuration weapon. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec weapon. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in weapon.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `weapon.json` dans le répertoire de votre projet :

   ```json title="weapon.json" ""myprovider"" {5-15}
   {
     "$schema": "https://weapon.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="weapon.json" {9,11,17-20}
{
  "$schema": "https://weapon.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à Weapon de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `weapon auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration weapon et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration weapon.
