...
description: Créez des outils que le LLM peut appeler dans weapon.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de weapon comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.weapon/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/weapon/tools/`.
...
```ts title=".weapon/tools/database.ts" {1}
...
```ts title=".weapon/tools/project.ts" {8}
import { tool } from "@weapon-ai/plugin"
...
```python title=".weapon/tools/add.py"
import sys
...
```ts title=".weapon/tools/python-add.ts" {10}
import { tool } from "@weapon-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".weapon/tools/add.py")
