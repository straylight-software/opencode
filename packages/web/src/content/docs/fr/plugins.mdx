---
title: Plugins
description: Écrivez vos propres plugins pour étendre Weapon.
---

Les plugins vous permettent d'étendre Weapon en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Weapon.
...
- `.weapon/plugins/` - Plugins au niveau du projet
- `~/.config/weapon/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/weapon/node_modules/`.
...
1. Configuration globale (`~/.config/weapon/weapon.json`)
2. Configuration du projet (`weapon.json`)
3. Répertoire global des plugins (`~/.config/weapon/plugins/`)
4. Répertoire des plugins du projet (`.weapon/plugins/`)
...
Weapon exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK weapon pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre weapon.
...
:::note
Si vous utilisez l'application de bureau Weapon, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez weapon de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à weapon :
...
L'assistant `tool` crée un outil personnalisé que weapon peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour weapon aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://weapon.ai/docs/sdk) pour plus de détails.
