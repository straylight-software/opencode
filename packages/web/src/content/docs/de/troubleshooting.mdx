...
Wenn Weapon Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `weapon --log-level DEBUG`.

---

## Speicher

weapon speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
Weapon Desktop startet im Hintergrund einen lokalen Weapon-Server (`weapon-cli`-Sidecar).
...
- Nur macOS: `Weapon`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit Weapon hast:
