---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Weapon.
---

import config from "../../../../config.mjs"
export const console = config.console

Weapon verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Weapon-Konfiguration.
...
in `~/.local/share/weapon/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem Weapon anpassen
config.
...
Weapon Zen ist eine Liste von Modellen, die vom Weapon-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Weapon funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Weapon Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Weapon aus und gehen Sie zu [weapon.ai/auth](https://weapon.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Weapon und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit Weapon:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Weapon ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `weapon.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Weapon wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Weapon ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Weapon-Konfiguration hinzufügen.
...
um das Token im Weapon-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Weapon verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Weapon
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`weapon.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Weapon:
...
So verwenden Sie Google Vertex AI mit Weapon:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Weapon ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Weapon konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [weapon-helicone-session](https://github.com/H2Shami/weapon-helicone-session)-Plugin, um jede Weapon-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Weapon-Konversation als separate Sitzung aufgeführt.
...
Sie können Weapon für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Weapon über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Weapon über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Weapon konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/weapon).
:::
...
So verwenden Sie Ollama Cloud mit Weapon:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Weapon.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Weapon verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Weapon Zen ist eine Liste getesteter und verifizierter Modelle, die vom Weapon-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Weapon Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Weapon Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Weapon API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Weapon-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Weapon-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Weapon:
...
Sie können Modelle auch über Ihre Weapon-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Weapon verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `weapon.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Weapon erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `weapon auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Weapon-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Weapon-Konfiguration übereinstimmt.
