---
title: Regeln
description: Lege eigene Anweisungen fuer weapon fest.
---

Du kannst weapon ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in weapon den Befehl `/init` aus.
...
So kann weapon sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht weapon sie zu erweitern.
...
weapon kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/weapon/AGENTS.md` speichern.
Diese gelten in allen weapon-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Weapon dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/weapon/AGENTS.md` existiert)
...
Beim Start sucht weapon in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/weapon/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `weapon.json` oder in `~/.config/weapon/weapon.json` angeben.
...
weapon parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit weapon.json

Empfohlen ist das `instructions`-Feld in der `weapon.json`:
...
Du kannst weapon explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `weapon.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
