...
Agent Skills erlauben Weapon, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Weapon sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Weapon vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.weapon/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/weapon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.weapon/skills/git-release/SKILL.md` zum Beispiel so:
...
Weapon listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `weapon.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `weapon.json`):
