---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Weapon zu erweitern.
---

Mit Plugins können Sie Weapon erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Weapon zu ändern.
...
- `.weapon/plugins/` – Plugins auf Projektebene
- `~/.config/weapon/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/weapon/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/weapon/weapon.json`)
2. Projektkonfiguration (`weapon.json`)
3. Globales Plugin-Verzeichnis (`~/.config/weapon/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.weapon/plugins/`)
...
Weapon führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Weapon-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Weapon erweitern können.
...
:::note
Wenn Sie die Weapon-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Weapon `.env`-Dateien liest:
...
Plugins können Weapon auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Weapon aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Weapon zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://weapon.ai/docs/sdk).
