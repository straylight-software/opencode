---
title: Server
description: Nutze den weapon-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `weapon serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein weapon-Client nutzen kann.
...
Setze `WEAPON_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `weapon`, kann aber mit `WEAPON_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `weapon serve` und `weapon web`.
...
Wenn du `weapon` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den weapon-Server, um weapon programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `weapon serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `weapon serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Weapon-[IDE](/docs/ide)-Plugins.
...
Der weapon-Server stellt folgende APIs bereit.
