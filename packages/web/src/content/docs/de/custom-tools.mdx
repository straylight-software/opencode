...
description: Erstellen Sie Tools, die der LLM in Weapon aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Weapon wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.weapon/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/weapon/tools/` platzieren.
