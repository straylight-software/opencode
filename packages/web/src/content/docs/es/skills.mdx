...
Las habilidades del agente permiten a Weapon descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Weapon busca estas ubicaciones:

- Configuración del proyecto: `.weapon/skills/<name>/SKILL.md`
- Configuración global: `~/.config/weapon/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Weapon sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.weapon/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/weapon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.weapon/skills/git-release/SKILL.md` así:
...
Weapon enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `weapon.json`:
...
**Para agentes integrados** (en `weapon.json`):
