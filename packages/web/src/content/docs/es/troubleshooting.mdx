...
Para depurar problemas con Weapon, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/weapon/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\weapon\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `weapon --log-level DEBUG`.

---

## Almacenamiento

weapon almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/weapon/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\weapon`
...
Weapon Desktop ejecuta un servidor Weapon local (el sidecar `weapon-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Weapon` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/weapon/weapon.jsonc` (o `~/.config/weapon/weapon.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/weapon/weapon.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\weapon\weapon.jsonc`
...
Weapon también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/weapon/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\weapon\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Weapon pueda reconstruirlo.

1. Salga de Weapon Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/weapon`
- **Linux**: eliminar `~/.cache/weapon` (o ejecutar `rm -rf ~/.cache/weapon`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\weapon`

3. Reinicie el escritorio Weapon.

---

### Solucionar problemas de conexión del servidor

Weapon Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `weapon.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `WEAPON_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `WEAPON_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Weapon requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Weapon.

---

### Notificaciones que no se muestran

Weapon Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Weapon en la configuración de su sistema operativo, y
...
1. Salga del escritorio Weapon.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Weapon):

- `weapon.settings.dat` (URL del servidor predeterminado de escritorio)
- `weapon.global.dat` y `weapon.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/weapon/issues**](https://github.com/anomalyco/weapon/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**weapon.ai/discord**](https://weapon.ai/discord)
...
3. Asegúrese de tener la última versión con `weapon upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `weapon models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/weapon
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\weapon`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. weapon instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\weapon`

2. Reinicie weapon para reinstalar los paquetes del proveedor más recientes.

Esto obligará a weapon a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
weapon detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
