---
title: Servidor
description: Interactuar con el servidor weapon a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `weapon serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente weapon puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `weapon.local` |
...
Configure `WEAPON_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `weapon`, o configure `WEAPON_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `weapon serve` como a `weapon web`.
...
Cuando ejecuta `weapon`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor weapon para interactuar con weapon mediante programación.
:::

Esta arquitectura permite que weapon admita múltiples clientes y le permite interactuar con weapon mediante programación.

Puede ejecutar `weapon serve` para iniciar un servidor independiente. Si tienes el
weapon TUI ejecutándose, `weapon serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Weapon [IDE](/docs/ide).
...
El servidor weapon expone las siguientes API.
