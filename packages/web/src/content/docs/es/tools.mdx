...
Las herramientas permiten que LLM realice acciones en su código base. Weapon viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Weapon.
...
:::note
Esta herramienta solo está disponible cuando `WEAPON_EXPERIMENTAL_LSP_TOOL=true` (o `WEAPON_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Weapon o cuando la variable de entorno `WEAPON_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Weapon:

```bash
WEAPON_ENABLE_EXA=1 weapon
```
