---
title: Complementos
description: Escriba sus propios complementos para extender Weapon.
---

Los complementos le permiten extender Weapon al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Weapon.
...
- `.weapon/plugins/` - Complementos a nivel de proyecto
- `~/.config/weapon/plugins/` - Complementos globales
...
```json title="weapon.json"
{
  ...
  "plugin": ["weapon-helicone-session", "weapon-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/weapon/node_modules/`.
...
1. Configuración global (`~/.config/weapon/weapon.json`)
2. Configuración del proyecto (`weapon.json`)
3. Directorio global de complementos (`~/.config/weapon/plugins/`)
4. Directorio de complementos del proyecto (`.weapon/plugins/`)
...
```json title=".weapon/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Weapon ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".weapon/plugins/my-plugin.ts"
...
```js title=".weapon/plugins/example.js"
...
- `client`: Un cliente SDK weapon para interactuar con la IA.
...
import type { Plugin } from "@weapon-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar weapon.
...
```js title=".weapon/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "weapon"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Weapon, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que weapon lea archivos `.env`:

```javascript title=".weapon/plugins/env-protection.js"
...
```javascript title=".weapon/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a weapon:

```ts title=".weapon/plugins/custom-tools.ts"
import { type Plugin, tool } from "@weapon-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que weapon puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para weapon junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://weapon.ai/docs/sdk) para obtener más detalles.
...
```ts title=".weapon/plugins/compaction.ts"
import type { Plugin } from "@weapon-ai/plugin"
...
```ts title=".weapon/plugins/custom-compaction.ts"
import type { Plugin } from "@weapon-ai/plugin"
