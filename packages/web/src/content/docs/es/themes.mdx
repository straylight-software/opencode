...
Con Weapon puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Weapon usa nuestro propio tema `weapon`.
...
Weapon viene con varios temas integrados.
...
- Quiere que Weapon coincida con la apariencia de su terminal
...
Weapon admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/weapon/themes/*.json` o `$XDG_CONFIG_HOME/weapon/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.weapon/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.weapon/themes/*.json`
