...
Weapon에는 두 가지 유형의 에이전트가 있습니다: 기본 에이전트(Primary Agent)와 서브 에이전트(Subagent).
...
Weapon는 두 가지 내장 기본 에이전트, **Build** 및 **Plan**을 제공합니다. 아래에서 자세히 살펴봅니다.
...
Weapon는 두 가지 내장 서브 에이전트, **General** 및 **Explore**를 제공합니다. 아래에서 자세히 살펴봅니다.

---

## 기본 제공

Weapon는 기본 에이전트와 두 개의 내장 서브 에이전트를 제공합니다.
...
`weapon.json` config 파일에 에이전트 구성:
...
온도가 지정되지 않은 경우, Weapon는 모델별 기본값을 사용합니다. 일반적으로 대부분의 모델은 0, Qwen 모델의 경우 0.55입니다.
...
이 경로는 구성 파일이 있는 위치를 기준으로 합니다. 따라서 글로벌 Weapon 구성과 프로젝트별 구성 모두에서 작동합니다.
...
Weapon 구성의 모델 ID는 `provider/model-id` 형식을 사용합니다. 예를 들어, [Weapon Zen](/docs/zen)을 사용한다면, GPT 5.1 Codex에 `weapon/gpt-5.1-codex`를 사용할 수 있습니다.
...
:::tip
`weapon models`를 실행하여 사용 가능한 모델 목록을 볼 수 있습니다.
:::
...
:::tip
공유하고 싶은 에이전트가 있습니까? [PR](https://github.com/anomalyco/weapon).
:::
