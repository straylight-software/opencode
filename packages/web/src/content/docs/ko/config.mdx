---
title: 구성
description: Weapon JSON 구성을 사용합니다.
---

JSON 구성 파일을 사용하여 Weapon를 구성할 수 있습니다.

---

## 형식

Weapon는 **JSON** 및 **JSONC** (주석이 있는 JSON) 형식을 지원합니다.
...
예를 들어, 전역 구성이 `theme: "weapon"` 및 `autoupdate: true`를 설정하고 프로젝트 구성이 `model: "anthropic/claude-sonnet-4-5"`를 설정하면 최종 구성에는 세 가지 설정이 모두 포함됩니다.
...
1. **원격 구성** (`.well-known/weapon`에서) - 조직 기본값
2. **전역 구성** (`~/.config/weapon/weapon.json`) - 사용자 환경설정
3. **사용자 정의 구성** (`WEAPON_CONFIG` 환경 변수) - 사용자 정의 재정의
4. **프로젝트별 구성** (`weapon.json`) - 프로젝트별 설정
5. **`.weapon` 디렉토리** - 에이전트, 명령, 플러그인
6. **인라인 구성** (`WEAPON_CONFIG_CONTENT` 환경 변수) - 런타임 재정의

이것은 프로젝트 구성이 전역 기본값을 덮어쓸 수 있고, 전역 구성이 원격 조직 기본값을 덮어쓸 수 있음을 의미합니다.

:::note
`.weapon`와 `~/.config/weapon` 디렉토리는 하위 디렉토리에 대해 **복수형 이름**을 사용합니다: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, 그리고 `themes/`. 단수형 이름(예: `agent/`)도 하위 호환성을 위해 지원됩니다.
:::
...
조직은 `.well-known/weapon` 엔드포인트를 통해 기본 구성을 제공할 수 있습니다. 이를 지원하는 공급자로 인증할 때 자동으로 가져옵니다.
...
`~/.config/weapon/weapon.json`에 전역 Weapon 구성을 배치합니다. 테마, 공급자, 키바인드와 같은 사용자 전체 기본 설정에 전역 구성을 사용하십시오.
...
프로젝트 루트에 `weapon.json`을 추가합니다. 프로젝트 구성은 표준 구성 파일 중 가장 높은 우선순위를 가집니다. 이는 전역 및 원격 구성을 모두 덮어씁니다.

:::tip
프로젝트의 루트에 특정 설정을 둡니다.
:::

Weapon가 시작될 때, 현재 디렉토리의 설정 파일이나 가장 가까운 Git 디렉토리를 찾습니다.
...
`WEAPON_CONFIG` 환경 변수를 사용하여 사용자 정의 구성 파일 경로를 지정합니다.
...
`WEAPON_CONFIG_DIR` 환경 변수를 사용하여 사용자 정의 구성 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.weapon` 디렉토리와 마찬가지로 에이전트, 명령, 모드 및 플러그인을 검색하며 동일한 구조를 따라야 합니다.
...
사용자 정의 디렉토리는 전역 구성 이후 및 `.weapon` 디렉토리 이전에 로드됩니다.

---

## 스키마

구성 파일에는 [**`weapon.ai/config.json`**](https://weapon.ai/config.json)에 정의된 스키마가 있습니다.
...
`weapon serve` 및 `weapon web` 명령에 대한 서버 설정을 구성할 수 있습니다.
...
- `mdns` - mDNS 서비스 발견 활성화. 로컬 네트워크의 다른 장치가 Weapon 서버를 찾을 수 있습니다.
- `mdnsDomain` - mDNS 서비스를 위한 사용자 정의 도메인 이름. 기본값은 `weapon.local`입니다. 동일한 네트워크에서 여러 인스턴스를 실행할 때 유용합니다.
...
`provider`, `model`, `small_model` 옵션을 통해 Weapon 구성에서 사용할 공급자와 모델을 구성할 수 있습니다.
...
`small_model` 옵션은 제목 생성과 같은 가벼운 작업을 위한 별도의 모델을 구성합니다. 기본적으로, Weapon는 공급자에게서 사용 가능한 더 저렴한 모델이 있다면 그것을 사용하고, 그렇지 않으면 주 모델로 돌아갑니다.
...
`theme` 옵션을 통해 Weapon 구성에서 사용할 테마를 설정할 수 있습니다.
...
`~/.config/weapon/agents/` 또는 `.weapon/agents/`에서 Markdown 파일을 사용하여 에이전트를 정의할 수도 있습니다. [더 알아보기](/docs/agents).
...
기본 에이전트는 기본(primary) 에이전트여야 합니다(서브 에이전트 불가). `"build"` 또는 `"plan"`과 같은 내장 에이전트이거나 정의된 [사용자 정의 에이전트](./agents)일 수 있습니다. 지정된 에이전트가 존재하지 않는 경우, Weapon는 경고와 함께 `"build"`로 돌아갑니다.

이 설정은 모든 인터페이스에 적용됩니다: TUI, CLI (`weapon run`), 데스크톱 앱 및 GitHub Action.
...
`~/.config/weapon/commands/` 또는 `.weapon/commands/`에서 Markdown 파일을 사용하여 명령을 정의할 수도 있습니다. [더 알아보기](/docs/commands).
...
Weapon는 시작될 때 자동으로 새로운 업데이트를 다운로드합니다. `autoupdate` 옵션으로 이를 비활성화할 수 있습니다.
...
기본적으로, Weapon는 **명시적 승인 없이 모든 작업을 허용**합니다. `permission` 옵션을 사용하여 이를 변경할 수 있습니다.
...
[플러그인](/docs/plugins)은 사용자 정의 도구, 훅(hook), 통합으로 Weapon를 확장합니다.

`.weapon/plugins/` 또는 `~/.config/weapon/plugins/`에 플러그인 파일을 배치하십시오. `plugin` 옵션을 통해 npm에서 플러그인을 로드할 수 있습니다.
...
Weapon를 제한하여 특정 공급자만 사용하도록 할 때 유용합니다.
