---
title: 사용자 정의 도구
description: Weapon에서 LLM이 호출할 수 있는 도구를 만듭니다.
---

사용자 정의 도구는 LLM이 대화 중에 호출 할 수있는 기능을 만듭니다. 그들은 `read`, `write` 및 `bash`와 같은 weapon의 [붙박이 도구](./tools)와 함께 작동합니다.
...
- 프로젝트의 `.weapon/tools/` 디렉토리에 배치하여 로컬.
- 또는 전 세계적으로 `~/.config/weapon/tools/`에 배치하여.
