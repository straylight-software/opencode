...
description: weapon, LSP sunucularınızla bütünleşir.
---

weapon, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

weapon, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`WEAPON_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP sunucularını, weapon yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
