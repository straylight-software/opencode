...
weapon ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\weapon\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `weapon --log-level DEBUG`.

---

## Depolama

weapon, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/weapon/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\weapon` yapistirin
...
weapon Desktop arka planda yerel bir weapon sunucusu (`weapon-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `weapon` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/weapon/weapon.jsonc` (veya `~/.config/weapon/weapon.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/weapon/weapon.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\weapon\weapon.jsonc` yapistirin
...
weapon diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/weapon/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\weapon\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), weapon'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. weapon Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/weapon`
- **Linux**: `~/.cache/weapon` dizinini silin (veya `rm -rf ~/.cache/weapon` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\weapon` yapistirin

3. weapon Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

weapon Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`weapon.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `WEAPON_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `WEAPON_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta weapon Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, weapon ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

weapon Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda weapon icin bildirimler etkinse
...
1. weapon Desktop'u kapatin
2. Su dosyalari bulun ve silin (weapon Desktop uygulama veri dizininde yer alirlar):

- `weapon.settings.dat` (desktop varsayilan sunucu URL'si)
- `weapon.global.dat` ve `weapon.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
weapon ile ilgili bir sorun yasiyorsaniz:
...
3. `weapon upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `weapon models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\weapon`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. weapon, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\weapon`

2. En guncel provider paketlerini yeniden kurmak icin weapon'u yeniden baslatin

Bu, weapon'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
weapon, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
