...
weapon'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
weapon, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
weapon, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

weapon iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `weapon.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse weapon modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global weapon yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
weapon hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [Weapon Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `weapon/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `weapon models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/weapon).
:::
