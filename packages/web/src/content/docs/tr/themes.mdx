...
weapon ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak weapon kendi `weapon` temasını kullanır.
...
weapon birden fazla yerleşik temayla gelir.
...
- weapon'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
weapon, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/weapon/themes/*.json` veya `$XDG_CONFIG_HOME/weapon/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.weapon/themes/*.json`
4. **Geçerli çalışma dizini** - `./.weapon/themes/*.json`
