---
title: Sunucu
description: weapon sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`weapon serve` komutu, weapon istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `weapon.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `WEAPON_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `weapon` değeridir; değiştirmek isterseniz `WEAPON_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `weapon serve` hem de `weapon web` için geçerlidir.
...
`weapon` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
weapon ile programatik etkileşim için weapon sunucusunu kullanın.
:::

Bu mimari, weapon'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`weapon serve` ile bağımsız bir sunucu başlatabilirsiniz. weapon TUI açıksa, `weapon serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum weapon [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
weapon sunucusu aşağıdaki API'leri sunar.
