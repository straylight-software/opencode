...
Ajan becerileri, weapon'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
weapon su konumlari tarar:

- Proje konfigurasyonu: `.weapon/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/weapon/skills/<name>/SKILL.md`
...
Proje yerel yollarinda weapon, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.weapon/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/weapon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.weapon/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
weapon, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`weapon.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`weapon.json` icinde):
