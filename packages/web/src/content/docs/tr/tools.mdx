...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. weapon yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
weapon içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `WEAPON_EXPERIMENTAL_LSP_TOOL=true` (veya `WEAPON_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca weapon provider kullanılırken veya `WEAPON_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

weapon başlatırken etkinleştirmek için:
