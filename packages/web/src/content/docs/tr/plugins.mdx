---
title: Eklentiler
description: weapon'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek weapon'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya weapon'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.weapon/plugins/` - Proje düzeyinde eklentiler
- `~/.config/weapon/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/weapon/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/weapon/weapon.json`)
2. Project config (`weapon.json`)
3. Global eklenti dizini (`~/.config/weapon/plugins/`)
4. Proje eklenti dizini (`.weapon/plugins/`)
...
weapon bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için weapon'lu bir SDK istemcisi.
...
weapon'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
weapon masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
weapon'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca weapon'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, weapon'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK documentation](https://weapon.ai/docs/sdk)'e bakın.
