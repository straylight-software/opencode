---
title: Sağlayıcılar
description: weapon'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

weapon, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı weapon yapılandırmanızda yapılandırın.
...
`~/.local/share/weapon/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları weapon'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Weapon Zen, weapon ekibi tarafından sağlanan modellerin bir listesidir.
weapon ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Weapon Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, weapon'u seçin ve [weapon.ai/auth](https://weapon.ai/auth)'ye gidin.
...
weapon'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u weapon ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   weapon'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `weapon.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi weapon'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   weapon'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Weapon yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in weapon auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
weapon, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. weapon'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`weapon.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`weapon.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi weapon'la kullanmak için:
...
Google Vertex AI'yi weapon ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları weapon çalıştırılırken ayarlayın.
...
Helicone'un weapon aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her weapon görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [weapon-helicone-session](https://github.com/H2Shami/weapon-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her weapon görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
weapon'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla weapon'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla weapon'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini weapon için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/weapon)'a bakın.
:::
...
Ollama Cloud'u weapon ile kullanmak için:
...
3. weapon'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: weapon'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Weapon Zen, weapon ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Weapon Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Weapon Zen**'i arayın.

   ```txt
   /connect
   ```

3. weapon API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   weapon yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca weapon yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Weapon ile kullanmak için:
...
Modelleri weapon yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı weapon ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `weapon.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları weapon'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `weapon auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için weapon yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, weapon yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
