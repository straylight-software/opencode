---
title: Server
description: Interact with weapon server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `weapon serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an weapon client can use.
...
Set `WEAPON_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `weapon`, or set `WEAPON_SERVER_USERNAME` to override it. This applies to both `weapon serve` and `weapon web`.
...
When you run `weapon` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the weapon server to interact with weapon programmatically.
:::

This architecture lets weapon support multiple clients and allows you to interact with weapon programmatically.

You can run `weapon serve` to start a standalone server. If you have the
weapon TUI running, `weapon serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Weapon [IDE](/docs/ide) plugins.
...
The weapon server exposes the following APIs.
