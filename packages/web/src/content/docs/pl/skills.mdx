...
Umiejętności agenta pozwalają weapon odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
weapon przeszukuje te lokalizacje:

- Project config: `.weapon/skills/<name>/SKILL.md`
- Global config: `~/.config/weapon/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu weapon przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.weapon/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/weapon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.weapon/skills/git-release/SKILL.md` w ten sposób:
...
weapon wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `weapon.json`:
...
**Dla agentów wbudowanych** (w `weapon.json`):
