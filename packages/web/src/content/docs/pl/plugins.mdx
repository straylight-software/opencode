---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć weapon.
---

Wtyczki umożliwiające wyłączenie weapon poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania weapon.
...
- `.weapon/plugins/` - Wtyczki na poziomie projektu
- `~/.config/weapon/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/weapon/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/weapon/weapon.json`)
2. Konfiguracja projektu (`weapon.json`)
3. Globalny katalog wtyczek (`~/.config/weapon/plugins/`)
4. Katalog wtyczek projektu (`.weapon/plugins/`)
...
weapon uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z weapon do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do weapon.
...
:::note
Jeśli korzystasz z aplikacji komputerowej weapon, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj weapon czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do weapon:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod weapon. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://weapon.ai/docs/sdk).
