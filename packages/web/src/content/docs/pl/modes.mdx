...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji weapon.
...
te lub skonfiguruj własne za pomocą konfiguracji weapon.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.weapon/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `weapon.json`:
...
Jeśli nie ma wpływu na temperaturę, weapon stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna weapon, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.weapon/modes/` dla trybów zapisanych dla projektu lub `~/.config/weapon/modes/` dla trybów globalnych:
