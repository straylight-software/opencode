...
W Weapon obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Weapon zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Weapon ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Weapon ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `weapon.json`:
...
- Globalnie: `~/.config/weapon/agents/`
- W projekcie: `.weapon/agents/`
...
Jeśli nie zostanie skonfigurowana, Weapon używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Weapon, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Weapon w `provider/model-id`. Na przykład, [Weapon Zen](/docs/zen), użyj `weapon/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `weapon models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/weapon).
:::
