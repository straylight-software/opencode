...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Weapon posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Weapon.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `WEAPON_EXPERIMENTAL_LSP_TOOL=true` (lub `WEAPON_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Weapon lub gdy zmienna środowiskowa `WEAPON_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Weapon:
