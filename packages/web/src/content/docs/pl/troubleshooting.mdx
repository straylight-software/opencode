...
Aby debugować problemy z weapon, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\weapon\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `weapon --log-level DEBUG`.

---

## Przechowywanie danych

weapon przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/weapon/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\weapon`
...
weapon Desktop uruchamia w tle lokalny serwer weapon (sidecar `weapon-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Weapon` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/weapon/weapon.jsonc` (lub `~/.config/weapon/weapon.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/weapon/weapon.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\weapon\weapon.jsonc`
...
weapon może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/weapon/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\weapon\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby weapon mógł ją odbudować.

1. Całkowicie zamknij weapon Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/weapon`
- **Linux**: usuń `~/.cache/weapon` (lub uruchom `rm -rf ~/.cache/weapon`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\weapon`

3. Uruchom ponownie weapon Desktop.

---

### Napraw problemy z połączeniem z serwerem

weapon Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `weapon.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `WEAPON_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `WEAPON_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows weapon Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami weapon.

---

### Brak powiadomień

weapon Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla weapon w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie weapon Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji weapon Desktop):

- `weapon.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `weapon.global.dat` i `weapon.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `weapon upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `weapon models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\weapon`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. weapon dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\weapon`

2. Uruchom ponownie kod weapon, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to weapon do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
weapon wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
