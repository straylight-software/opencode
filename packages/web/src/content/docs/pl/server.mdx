---
title: Serwer
description: Interakcja z serwerem weapon za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `weapon serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient weapon.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `weapon.local` |
...
Ustaw `WEAPON_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `weapon` lub ustaw `WEAPON_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `weapon serve`, jak i `weapon web`.
...
Kiedy uruchomisz `weapon`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera weapon do programowej interakcji z weapon.
:::

Ta architektura umożliwia obsługę wielu klientów przez weapon i programową interakcję z kodem otwartym.

Możesz uruchomić `weapon serve`, aby uruchomić samodzielny serwer. Jeśli masz
weapon TUI działa, `weapon serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki weapon [IDE](/docs/ide).
...
Serwer weapon udostępnia następujące interfejsy API.
