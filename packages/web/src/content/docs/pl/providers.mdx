---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w weapon.
---

import config from "../../../../config.mjs"
export const console = config.console

weapon używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji weapon.
...
w `~/.local/share/weapon/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim weapon
config.
...
Weapon Zen to lista modeli dostarczonych przez zespół weapon, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z weapon. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Weapon Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz weapon i przejdź do [weapon.ai/auth](https://weapon.ai/auth).
...
Działa jak każdy inny dostawca w weapon i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z weapon:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania weapon:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `weapon.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w weapon nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod weapon działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji weapon.
...
weapon używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować weapon
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`weapon.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem weapon:
...
Aby używać Google Vertex AI z weapon:
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą weapon, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [weapon-helicone-session](https://github.com/H2Shami/weapon-helicone-session), aby automatycznie rejestrować każdą konwersację weapon jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja weapon będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować weapon tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować weapon tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować weapon tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem weapon. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/weapon).
:::
...
Aby korzystać z Ollama Cloud z weapon:
...
3. Skopiuj klucz API do użycia w weapon.
...
6. **Ważne**: Przed użyciem modeli chmurowych w weapon musisz pobrać informacje o modelu lokalnie:
...
Weapon Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół weapon. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Weapon Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Weapon Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API weapon.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji weapon.
...
5. Możesz je także dostosować za pomocą konfiguracji weapon. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z weapon:
...
Możesz także dostosować modele za pomocą konfiguracji weapon. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z weapon. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `weapon.json` w katalogu projektu:
...
Pola `limit` pozwalają weapon zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `weapon auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację weapon i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji weapon.
