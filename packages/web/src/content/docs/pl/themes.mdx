...
Dzięki weapon możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie weapon używa naszego własnego motywu `weapon`.
...
weapon ma kilka wbudowanych motywów.
...
- Want weapon to match their terminal's appearance
...
weapon obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/weapon/themes/*.json` lub `$XDG_CONFIG_HOME/weapon/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.weapon/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.weapon/themes/*.json`
