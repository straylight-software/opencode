---
title: Providers
description: Using any LLM provider in Weapon.
---

import config from "../../../config.mjs"
export const console = config.console

Weapon uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Weapon config.
...
You can customize the providers through the `provider` section in your Weapon
config.
...
Weapon Zen is a list of models provided by the Weapon team that have been
tested and verified to work well with Weapon. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Weapon Zen.
:::

1. Run the `/connect` command in the TUI, select weapon, and head to [weapon.ai/auth](https://weapon.ai/auth).
...
It works like any other provider in Weapon and is completely optional to use.
...
To use Amazon Bedrock with Weapon:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running weapon:
   ...
   For project-specific or persistent configuration, use `weapon.json`:
...
:::info
Using your Claude Pro/Max subscription in Weapon is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for weapon to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your weapon config.
...
Weapon uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Weapon
to only use your own GitLab-hosted instance, add the following to your
`weapon.json` file. It is also recommended to disable session sharing.
...
To use your GitHub Copilot subscription with weapon:
...
To use Google Vertex AI with Weapon:
...
In the event you see a feature or model from Helicone that isn't configured automatically through weapon, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [weapon-helicone-session](https://github.com/H2Shami/weapon-helicone-session) plugin to automatically log each Weapon conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Weapon conversation listed as a separate session.
...
You can configure weapon to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure weapon to use local models through LM Studio.
...
You can configure weapon to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Weapon. See the [Ollama integration docs](https://docs.ollama.com/integrations/weapon) for details.
:::
...
To use Ollama Cloud with Weapon:
...
3. Copy the API key for use in Weapon.
...
6. **Important**: Before using cloud models in Weapon, you must pull the model information locally:
...
Weapon Zen is a list of tested and verified models provided by the Weapon team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Weapon Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Weapon Zen**.

   ```txt
   /connect
   ```

3. Enter your Weapon API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your weapon config.
...
5. You can also customize them through your weapon config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Weapon:
...
You can also customize models through your weapon config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with weapon. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `weapon.json` file in your project directory:
...
The `limit` fields allow Weapon to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `weapon auth list` to see if the credentials
...
2. For custom providers, check the weapon config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your weapon config.
