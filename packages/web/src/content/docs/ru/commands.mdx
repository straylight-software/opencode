...
Создайте `.weapon/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию weapon или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем weapon [config](/docs/config):
...
- Глобальный: `~/.config/weapon/commands/`
- Для каждого проекта: `.weapon/commands/`
...
weapon включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
