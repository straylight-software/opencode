...
const managedConfigDir = process.env.WEAPON_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "weapon.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "weapon.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://weapon.ai/config.json",
        model: "test/model",
        username: "testuser",
      })
    },
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "weapon.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "weapon.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .weapon directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const weaponDir = path.join(dir, ".weapon")
      await fs.mkdir(weaponDir, { recursive: true })
      const agentDir = path.join(weaponDir, "agent")
...
test("loads agents from .weapon/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const weaponDir = path.join(dir, ".weapon")
      await fs.mkdir(weaponDir, { recursive: true })

      const agentsDir = path.join(weaponDir, "agents")
...
test("loads commands from .weapon/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const weaponDir = path.join(dir, ".weapon")
      await fs.mkdir(weaponDir, { recursive: true })

      const commandDir = path.join(weaponDir, "command")
...
test("loads commands from .weapon/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const weaponDir = path.join(dir, ".weapon")
      await fs.mkdir(weaponDir, { recursive: true })

      const commandsDir = path.join(weaponDir, "commands")
...
test("does not try to install dependencies in read-only WEAPON_CONFIG_DIR", async () => {
  ...
  const prev = process.env.WEAPON_CONFIG_DIR
  process.env.WEAPON_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.WEAPON_CONFIG_DIR
    else process.env.WEAPON_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable WEAPON_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "weapon.json"),
        JSON.stringify({ $schema: "https://weapon.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "weapon.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .weapon config
      const projectDir = path.join(dir, "project")
      const weaponDir = path.join(projectDir, ".weapon")
      await fs.mkdir(weaponDir, { recursive: true })
      ...
      // Local .weapon config with different plugins
      await Bun.write(
        path.join(weaponDir, "weapon.json"),
        JSON.stringify({
          $schema: "https://weapon.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .weapon config with some overlapping plugins
      await Bun.write(
        path.join(weaponDir, "weapon.json"),
        JSON.stringify({
          $schema: "https://weapon.ai/config.json",
...
// Note: preload.ts sets WEAPON_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-weapon@2.4.3")).toBe("oh-my-weapon")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@weapon/plugin@2.0.0")).toBe("@weapon/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-weapon@2.4.3", "file:///project/.weapon/plugin/oh-my-weapon.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.weapon/plugin/oh-my-weapon.js")
  })
  ...
  test("local plugin directory overrides global weapon.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const weaponDir = path.join(projectDir, ".weapon")
        const pluginDir = path.join(weaponDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("WEAPON_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["WEAPON_DISABLE_PROJECT_CONFIG"]
    process.env["WEAPON_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["WEAPON_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["WEAPON_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .weapon/ directories when flag is set", async () => {
    const originalEnv = process.env["WEAPON_DISABLE_PROJECT_CONFIG"]
    process.env["WEAPON_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .weapon directory with a command
          const weaponDir = path.join(dir, ".weapon", "command")
          await fs.mkdir(weaponDir, { recursive: true })
          await Bun.write(path.join(weaponDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .weapon should NOT be in directories list
          const hasProjectWeapon = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectWeapon).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["WEAPON_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["WEAPON_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["WEAPON_CONFIG_DIR"]
      process.env["WEAPON_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["WEAPON_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["WEAPON_CONFIG_DIR"]
      } else {
        process.env["WEAPON_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("WEAPON_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["WEAPON_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["WEAPON_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["WEAPON_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["WEAPON_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from WEAPON_CONFIG_DIR, not project
...
// WEAPON_CONFIG_CONTENT should support {env:} and {file:} token substitution
// just like file-based config sources do.
describe("WEAPON_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in WEAPON_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["WEAPON_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["WEAPON_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://weapon.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["WEAPON_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["WEAPON_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in WEAPON_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["WEAPON_CONFIG_CONTENT"]
