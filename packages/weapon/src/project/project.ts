...
import { fn } from "@weapon-ai/util/fn"
  ...
  export async function fromDirectory(directory: string) {
    ...
    const { id, sandbox, worktree, vcs } = await iife(async () => {
      ...
      if (git) {
        ...
        let id = await Bun.file(path.join(git, "weapon"))
          .text()
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.WEAPON_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            void Bun.file(path.join(git, "weapon"))
              .write(id)
              .catch(() => undefined)
          }
        }
    ...
    if (Flag.WEAPON_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
