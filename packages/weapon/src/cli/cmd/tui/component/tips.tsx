...
const TIPS = [
  ...
  "Press {highlight}Tab{/highlight} to cycle between LOCKED and ARMED modes with auto-recovery timeouts",
  "Use {highlight}/undo{/highlight} to revert the last message and file changes",
  "Use {highlight}/redo{/highlight} to restore previously undone messages and file changes",
  "Run {highlight}/share{/highlight} to create a public link to your conversation at weapon.ai",
  ...
  "Switch to {highlight}LOCKED{/highlight} mode to get suggestions without making actual changes",
  "Use {highlight}@agent-name{/highlight} in prompts to invoke specialized subagents",
  "Press {highlight}Ctrl+X Right/Left{/highlight} to cycle through parent and child sessions",
  "Create {highlight}weapon.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/weapon/weapon.json{/highlight} for global config",
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.weapon/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.weapon/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "Weapon auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "Weapon uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.weapon/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.weapon/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Weapon from reading sensitive files",
  "Use {highlight}weapon run{/highlight} for non-interactive scripting",
  "Use {highlight}weapon --continue{/highlight} to resume the last session",
  "Use {highlight}weapon run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}weapon serve{/highlight} for headless API access to Weapon",
  "Use {highlight}weapon run --attach{/highlight} to connect to a running server",
  "Run {highlight}weapon upgrade{/highlight} to update to the latest version",
  "Run {highlight}weapon auth list{/highlight} to see all configured providers",
  "Run {highlight}weapon agent create{/highlight} for guided agent creation",
  "Use {highlight}/weapon{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}weapon github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/weapon fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.weapon/themes/{/highlight} directory",
  ...
  "Run {highlight}weapon debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/weapon{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with Weapon Zen for curated, tested models",
  ...
  "In {highlight}ARMED{/highlight} mode, stalled agents auto-reset after the configured timeout",
]
