...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run weapon",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for weapon session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found weapon session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`weapon import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting weapon...")
        UI.println()

        // Launch weapon TUI with session ID if available
        const { spawn } = await import("child_process")
        const weaponArgs = sessionId ? ["-s", sessionId] : []
        const weaponProcess = spawn("weapon", weaponArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          weaponProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`weapon exited with code ${code}`))
          })
          weaponProcess.on("error", reject)
        })
      },
    })
  },
