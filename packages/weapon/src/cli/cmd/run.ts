...
import { EOL } from "os"
import { createWeaponClient, type Message, type WeaponClient, type ToolPart } from "@weapon-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run weapon with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running weapon server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: WeaponClient) {
    ...
    async function share(sdk: WeaponClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.WEAPON_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: WeaponClient) {
    ...
    if (args.attach) {
      const sdk = createWeaponClient({ baseUrl: args.attach })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createWeaponClient({ baseUrl: "http://weapon.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
