...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall weapon and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Weapon")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g weapon-ai",
      pnpm: "pnpm uninstall -g weapon-ai",
      bun: "bun remove -g weapon-ai",
      yarn: "yarn global remove weapon-ai",
      brew: "brew uninstall weapon",
      choco: "choco uninstall weapon",
      scoop: "scoop uninstall weapon",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall weapon -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using Weapon!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# weapon") || content.includes(".weapon/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".weapon/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".weapon/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".weapon"))
    ) {
      continue
    }
