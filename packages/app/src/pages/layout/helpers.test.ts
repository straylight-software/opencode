...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("weapon://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("weapon://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("weapon://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("weapon://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("weapon://open-project")).toBeUndefined()
    expect(parseDeepLink("weapon://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "weapon://open-project?directory=/a",
      "weapon://other?directory=/b",
      "weapon://open-project?directory=/c",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __WEAPON__: {
        deepLinks: ["weapon://open-project?directory=/a"],
      },
    } as unknown as Window & { __WEAPON__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["weapon://open-project?directory=/a"])
