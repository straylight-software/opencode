...
export const { use: useGlobalSDK, provider: GlobalSDKProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const password = typeof window === "undefined" ? undefined : window.__WEAPON__?.serverPassword

    const auth = (() => {
      if (!password) return
      if (!server.isLocal()) return
      return {
        Authorization: `Basic ${btoa(`weapon:${password}`)}`,
      }
    })()
    ...
    const eventSdk = createWeaponClient({
      baseUrl: server.url,
      signal: abort.signal,
      fetch: eventFetch,
