import { base64Decode } from "@weapon-ai/util/encode"
...
test("non-git projects keep workspace mode disabled", async ({ page, withProject }) => {
  ...
  const nonGit = await fs.mkdtemp(path.join(os.tmpdir(), "weapon-e2e-project-nongit-"))
  ...
  try {
    await withProject(async () => {
      ...
      expect(path.basename(activeDir)).toContain("weapon-e2e-project-nongit-")
